    ...
    public class VendaApiIntegrationTests
    {
        ...
        public async Task Should_Create_Sale_With_10PercentDiscount()
        {
                    ...
                    Items = new List<object>
            {
                ...
                response.EnsureSuccessStatusCode();
                Assert.NotNull(result);
                result.Should().Contain("\"valueMonetaryTaxApplied\":2.5");
                result.Should().Contain("27.5");
            }
        }
        ...
        {
            {
                ...
                var result = await response.Content.ReadAsStringAsync();
                Assert.NotNull(result);
                result.Should().Contain("\"valueMonetaryTaxApplied\":4.5");
                result.Should().Contain("\"total\":49.5");
                result.Should().Contain("\"valueMonetaryTaxApplied\":11");
                result.Should().Contain("\"total\":66.0");
                result.Should().Contain("115.5");
            }
        }
        ...
        {
            {
                ...
                result.Should().Contain("\"valueMonetaryTaxApplied\":11");
                result.Should().Contain("\"total\":66.0");
                result.Should().Contain("\"valueMonetaryTaxApplied\":20");
                result.Should().Contain("\"total\":120.0");
                result.Should().Contain("235.5");
            }
        }
